...
export function useAssetData() {
  ...
  const mutateSync = useCallback((newData?: unknown) => {
    if (newData) setSyncStatus(newData as SyncStatusResponse);
    mutateDashboard();
  }, [mutateDashboard]);

  const mutateFundamentals = useCallback((newData?: unknown) => {
    if (newData) setFundamentalsStatus(newData as SyncStatusResponse);
    mutateDashboard();
  }, [mutateDashboard]);
