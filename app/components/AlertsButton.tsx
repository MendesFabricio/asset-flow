...
export const AlertsButton = ({ onFixAsset }: Props) => {
  ...
  useEffect(() => {
    // Adiar com setTimeout remove o erro de render em cascata (set-state-in-effect)
  ...
  // Helpers Visuais Modernos como Objeto evita erro de chaves ausentes no array (jsx-key)
  const getSeverityStyles = (severity: number, type: string) => {
    if (severity === 5) {
      return {
        ...
        borderClass: "border-red-500/80 shadow-[inset_3px_0_0_0_rgba(239,68,68,0.6)]",
        hoverClass: "hover:bg-red-950/20"
      };
    }
    if (severity === 4) {
      return {
        icon: <AlertTriangle size={18} className="text-amber-500 drop-shadow-[0_0_3px_rgba(245,158,11,0.5)]" />,
        borderClass: "border-amber-500/80 shadow-[inset_3px_0_0_0_rgba(245,158,11,0.6)]",
    ...
    if (type === 'CONFIG') {
      return {
        ...
        borderClass: "border-blue-400/80 shadow-[inset_3px_0_0_0_rgba(96,165,250,0.6)]",
        hoverClass: "hover:bg-blue-950/20"
      };
    }
    if (type === 'OPORTUNIDADE') {
      return {
        icon: <RefreshCw size={18} className="text-emerald-400 drop-shadow-[0_0_3px_rgba(52,211,153,0.5)]" />,
        borderClass: "border-emerald-400/80 shadow-[inset_3px_0_0_0_rgba(52,211,153,0.6)]",
    ...
    if (type === 'NOVIDADE') {
      return {
        ...
        borderClass: "border-purple-400/80 shadow-[inset_3px_0_0_0_rgba(192,132,252,0.6)]",
        hoverClass: "hover:bg-purple-950/20"
      };
    }
    return {
      icon: <Info size={18} className="text-slate-400" />,
      borderClass: "border-slate-600/50",
      hoverClass: "hover:bg-slate-800/50"
    };
  };
