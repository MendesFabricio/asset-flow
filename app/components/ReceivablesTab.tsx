...
export const ReceivablesTab = () => {
    ...
    const handleEdit = (item: any) => { setEditingId(item.id); setDesc(item.descricao); setVal(item.valor_total.toString()); setParc(item.total_parcelas.toString()); setWho(item.devedor); setIsModalOpen(true); };
    ...
    const monthsData = Array.from({ length: 7 }, (_, i) => {
        ...
        const monthItems = filteredItems.map(item => {
        ...
        <div className="space-y-6 animate-in fade-in pb-10">
            ...
            <div className="flex flex-col gap-4 bg-slate-900 p-4 rounded-xl border border-slate-800">
                <div className="flex flex-col md:flex-row justify-between items-center gap-4">
                    ...
                    <button onClick={openNewModal} className="w-full md:w-auto bg-blue-600 hover:bg-blue-500 text-white px-4 py-2 rounded-lg flex items-center justify-center gap-2 text-sm font-bold shadow-lg shadow-blue-900/20 transition-all"><Plus size={16} /> Novo</button>
                </div>
                ...
                {debtors.length > 1 && (
                    <div className="flex items-center gap-2 pt-2 border-t border-slate-800/50 overflow-x-auto pb-2 scrollbar-thin">
                        ...
                        {debtors.map(person => (
                            <button key={person} onClick={() => setSelectedPerson(person)} className={`px-3 py-1 rounded-full text-xs font-bold transition-all whitespace-nowrap flex items-center gap-1 ${selectedPerson === person ? 'bg-blue-600 text-white shadow-md' : 'bg-slate-800 text-slate-400 hover:bg-slate-700 hover:text-slate-200'}`}>
                                    ...
                                    <div key={item.id} className={`p-3 rounded-lg border flex items-center justify-between group transition-all ${item.status === 'paid' ? 'bg-slate-900/20 border-slate-800/30 opacity-60' : 'bg-slate-950/50 border-slate-800/50 hover:border-slate-700'}`}>
                                        <div className="flex-1 min-w-0 mr-2">
                                            <div className="flex items-center gap-2">
                                                ...
                                                {item.status !== 'paid' && <button onClick={() => handleEdit(item)} className="text-slate-600 hover:text-blue-400 opacity-0 group-hover:opacity-100 transition-all p-1 rounded-md hover:bg-slate-800 shrink-0"><Pencil size={12} /></button>}
                                            </div>
                                        ...
                                        <div className="text-right flex items-center gap-3 shrink-0">
                                            ...
                                            {item.status === 'pending' && <button onClick={() => handlePay(item.id)} className="text-slate-500 hover:text-emerald-400 transition-colors p-1.5 rounded-full hover:bg-emerald-500/10" title="Receber agora"><CheckCircle size={20} /></button>}
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
                    <div className="bg-slate-900 p-6 rounded-xl border border-slate-700 w-full max-w-md space-y-5 shadow-2xl animate-in zoom-in-95 relative">
                        ...
                        <div className="flex gap-3 pt-2 border-t border-slate-800/50 mt-4">
                            {editingId && <button onClick={handleDelete} className="bg-red-500/10 hover:bg-red-500/20 text-red-500 p-3 rounded-lg font-bold border border-red-500/20 transition-colors flex items-center justify-center gap-2"><Trash2 size={18} /> <span className="hidden sm:inline">Excluir</span></button>}
                            <button onClick={handleSave} className="flex-1 bg-blue-600 hover:bg-blue-500 text-white p-3 rounded-lg font-bold shadow-lg shadow-blue-900/20 transition-all flex items-center justify-center gap-2"><CheckCircle size={18} /> {editingId ? 'Salvar' : 'Criar'}</button>
                        </div>
                    </div>
                </div>
            )}
