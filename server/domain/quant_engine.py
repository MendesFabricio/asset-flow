...
# ─── Novas Funções de Análise Quantitativa Avançada (Sprint 9) ───────────────

def calculate_kelly_criterion(session, fetch_prices) -> dict:
    """
    Calcula a alocação de ativos ideal usando o Critério de Kelly Fracionário (1/4 Kelly)
    com teto de 12% por ativo para renda variável.
    """
    from database.models import Position
    import pandas as pd
...
def calculate_alpha_attribution(session, fetch_prices) -> dict:
    """
    Decompõe o Alpha de Jensen anualizado do portfólio por ativo (CAPM).
    A contribuição ponderada total (Soma de w_i * alpha_i) é igual ao Alpha do portfólio.
    """
    from database.models import Position
    import pandas as pd
...
def calculate_rolling_sharpe(session, fetch_prices) -> dict:
    """
    Calcula a evolução temporal do Sharpe Ratio de cada ativo e da carteira consolidada
    em janelas móveis de 90 dias úteis de mercado.
    """
    from database.models import Position
    import pandas as pd
...
def calculate_momentum_ranking(session, fetch_prices) -> dict:
    """
    Calcula o ranking de momentum (12m - 1m) dos ativos de renda variável do portfólio.
    Formula: Retorno acumulado entre t-252 (12 meses) e t-21 (1 mês atrás).
    """
    from database.models import Position
    import pandas as pd
...
def calculate_efficient_frontier_points(session, fetch_prices) -> dict:
    """
    Simula 5.000 portfólios no backend e extrai:
    1. A nuvem de pontos (amostra de 150 pontos para o gráfico do Next.js).
    2. A curva da fronteira eficiente (30 pontos de menor risco agrupados por retorno).
    3. O ponto de Sharpe Máximo e o ponto de Menor Volatilidade (Min Variance).
    Salva a estrutura resultante na tabela SystemCache SQLite sob a chave 'efficient_frontier'.
    """
    from database.models import Position, SystemCache, safe_commit
    import pandas as pd
    import json
